...
vignette: >
  ...
  %\VignetteIndexEntry{warbleR: Acoustic (dis)similarity, coordinated singing and  and simulating songs}
...
frange.detec(readWave("Tinamus-major-154191-1.wav"), flim = c(0, 2.5), bp = c(0, 3), threshold = 15)
...
frange.detec(readWave("Tinamus-major-154191-1.wav"), flim = c(0, 2.5), bp = c(0, 3), threshold = 1)

```

<center> ![Frange low bandpass example](Tinamus-major-154191.wav-1-frange.jpeg)</center>
&nbsp; 
...
Spectral entropy can be calculated as time series in selected signals and plotted onto image files. Previously, spectral entropy was only available as a sole measurement across a selection, as measured by `specan`.
...
trackfreqs(Phae.hisnrt, wl = 300, flim = c(2, 10), bp = c(2, 12), col = c("purple", "orange"), pch = c(17, 3), res = 300, it = "jpeg")
...
df_inf <- inflections(X = df_df, parallel = 4, pb = TRUE)
str(df_inf)
...
<center> ![`compare.methods` image example](comp.meth-XCORR-dfDTW-3-20-22-31.jpeg)</center>
&nbsp; 
...
Phae.hisnrt <- read.csv("Phae_hisnrt.csv")
# here we will continue with only the selections from xeno-canto recordings
Phae.hisnrt <- droplevels(Phae.hisnrt[grep("Phaethornis", Phae.hisnrt$sound.files), ])
Phae.hisnrt <- droplevels(Phae.hisnrt[-grep("154123", Phae.hisnrt$sound.files), ])
...
# Add a song column to the data frame output from specan
...
# add song data to the specan data
sp <- merge(params, Phae.hisnrt, by = c("sound.files", "selec"))
str(sp)

# caculate song-level parameters for all numeric parameters
sp2 <- song_param(X = sp, song_colm = "song", parallel = 1, pb = TRUE)
str(sp2)
...
<center> ![Tinamou dfDTW image](Tinamus-major-154191.wav-1-dfDTW.jpeg)</center>
&nbsp; 
