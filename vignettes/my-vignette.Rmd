...
# Save metadata as a data frame object when download = FALSE 
Phae.anth <- querxc(qword = "Phaethornis anthophilus", download = FALSE) 

```

If you're interested in questions of geographic variation, you should use the function `xcmaps` to visualize the geographic range of the recordings prior to downloading. 
`xcmaps` will create a tiff file per species in your current directory, or in the plot window of RStudio if img = FALSE.

```{r, eval=FALSE}
?xcmaps

# image type can be changed to jpeg if tiff won't open
xcmaps(Phae, img = TRUE, it = "tiff") 
xcmaps(Phae.anth, img = TRUE, it = "tiff") 
...
write.csv(Phae.anth, "Phae.anth.csv", row.names = FALSE)
