...
# xeno-canto wav files must be remade into wav files with the argument extensible = FALSE in order to work in Raven 
wavs <- list.files(pattern = ".wav$")

invisible(lapply(1:length(wavs), function(x){
  writeWave(readWave(wavs[[x]]), filename = wavs[[x]], extensible = FALSE)
}))
