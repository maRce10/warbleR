...
Phae.hisnr <- sel_overlap(X = Phae.hisnr, max.ovlp = 0)

# run the function again but this time retain only the signals that don't overlap
Phae.hisnr <- sel_overlap(X = Phae.hisnr, max.ovlp = 0, drop = TRUE)
...
sel_tailor(Phae.hisnr2, wl = 300, flim = c(2,10), wn = "hanning", mar = 0.1, osci = TRUE, title = c("sound.files", "selec"), auto.next = TRUE)

# Read in sel_tailor output after renaming the csv file
