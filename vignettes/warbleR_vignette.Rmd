...
<br>
This vignette assumes a basic understanding of R, such as:

  1. **manipulating objects**
  2. **changing function arguments or using defaults**
  3. **writing to and reading from working directories**
  
_**Regular expression and apply functions make frequent appearances. If you're unfamiliar, read up on some basic regex and the apply family.**_
 
<br>
To start, we will use `warbleR` to download recordings from Xeno-Canto. If you have your own recordings and don't want to obtain sound files from Xeno-Canto, you can skip the first section and go to _Filter recordings by visual inspection_.
...
### Filter Xeno-Canto recordings by signal type
...
For example, we are interested in investigating the microgeographic variation of long-billed hermit (_Phaethornis longirostris_) songs. We should proceed by looking for a site with the highest number of songs. Our goal is to search for visible differences in song structure within a site, and then determine whether visual distinctiveness is representative of underlying differences in acoustic parameters. 
...
# Let's focus on the recordings from La Selva Biological Station
...
file.copy(from = file.path("/home/m/Documents/Biblioteca de cantos/Trochilidae/XC/wavs",fn),to=fn,overwrite = TRUE)
...
Xeno-Canto maintains recordings in `mp3` format, as this compressed format yields smaller files. However, we require `wav` format for all downstream analyses. Compression from `wav` to `mp3` and back involves information losses, but recordings that have undergone this transformation have been successfully used in research (Medina-Garcia et al. 2015, see references).
