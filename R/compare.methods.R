...
#' "dfDTW", "ffDTW", "SP"),it = "jpeg", parallel = 1, path = NULL, sp = NULL)
...
#' @param sp Data frame with acoustic parameters as the one generated by \code{specan}. Must contain 'sound.files'
...
compare.methods <- function(X = NULL, flim = c(0, 22), bp = c(0, 22), mar = 0.1, wl = 512, ovlp = 90, 
    ...
    it = "jpeg", parallel = 1, path = NULL, sp = NULL){  
  ...
  if("SP" %in% methods)
  { if(is.null(sp)) spmat <- specan(X, wl = 512, bp = flim, parallel = parallel) else spmat <- sp
  ...
  imgfun(filename = paste("comp.spec-", names(disim.mats)[1],"-",names(disim.mats)[2], "-", paste(X$labels, collapse = "-"), paste0(".", it), sep = ""), width = 16.25, height =  16.25, units = "cm", res = res)
