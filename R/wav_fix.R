...
#' 
#' \code{wav_fix} fixes sound files in .wav format so they can be imported into R.
#' @usage wav_fix(checksels = NULL, files = NULL, samp.rate = NULL, bit.depth = NULL,
#'  path = NULL, mono = FALSE)
#' @param checksels Data frame with results from \code{\link{sel_check}}. Default is \code{NULL}. If  both 'checksels' and 'files' are \code{NULL}
...
#' @export
#' @name wav_fix
#' @details This function aims to simplify the process of converting sound files that cannot be imported into R and/or homogenizing sound files. Problematic files can be determined using \code{\link{wav_check}} or \code{\link{sel_check}}. The  
#' \code{\link{sel_check}} output can be directly input using the argument 'checksels'. Alternatively a vector of file 
...
# 
# wav_fix(files = lbh_selec_table$sound.files, path = tempdir())
#' 
#' #check this folder
#' tempdir()
#' }
...
wav_fix <- function(checksels = NULL, files = NULL, samp.rate = NULL, bit.depth = NULL, path = NULL, mono = FALSE)
{
  ...
  argms <- methods::formalArgs(wav_fix)
...
##############################################################################################################
#' alternative name for \code{\link{wav_fix}}
#'
#' @keywords internal
#' @details see \code{\link{wav_fix}} for documentation. \code{\link{fixwavs}} will be deprecated in future versions.
#' @export
...
#'
#' @keywords internal
#' @details see \code{\link{wav_fix}} for documentation. \code{\link{fix_wavs}} will be deprecated in future versions.
#' @export
