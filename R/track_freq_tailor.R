...
#' 
#' \code{track_freq_tailor} produces an interactive spectrographic view (similar to \code{\link{manualoc}}) in 
#' which the start/end times and frequency range of acoustic signals listed in a data frame can be adjusted.
#' @usage track_freq_tailor(X = NULL, wl = 512, flim = c(0,22), wn = "hanning", mar = 0.5,
...
#'   title = c("sound.files", "selec"), ts.df = NULL, ...)
...
#' be used as the input data frame. Other data frames can be used as input, but must have at least the 4 columns mentioned above. Notice that, if an output file ("track_freq_tailor_output.csv") is found in the working directory it will be given priority over an input data frame.
...
#' 
#' track_freq_tailor(X =  selec.table, flim = c(1,12), wl = 300, auto.next = TRUE)
#' 
#' # Read output .csv file
#' track_freq_tailor.df <- read.csv("track_freq_tailor_output.csv")
#' track_freq_tailor.df
#' 
...
#'    selection that is adjusted. The function produces a .csv file (track_freq_tailor_output.csv) 
...
track_freq_tailor <- function(X = NULL, wl = 512, flim = c(0,22), wn = "hanning", mar = 0.5,
                      ...
                      collevels = NULL, title = c("sound.files", "selec"), ts.df = NULL, ...)
{
  ...
  if(!file.exists(file.path(getwd(), "track_freq_tailor_output.csv")))
  {X$tailored <- ""
  X$tailored <- as.character(X$tailored)
  if(!is.null(index))   X$tailored[!1:nrow(X) %in% index] <- "y"
  write.csv(droplevels(X[X$tailored != "delete", ]), "track_freq_tailor_output.csv", row.names =  FALSE)  
  } else {X <- read.csv("track_freq_tailor_output.csv", stringsAsFactors = FALSE)  
  ...
  repeat{
    ...
    labels <- c("stop", "next", "previous", "delete", "fix_freq")
    mrg <- 0.05
    ...
    if(xy$x > min(xs) & xy$x < max(xs) & xy$y > min(ys[[5]]) & xy$y < max(ys[[5]]))
    {
       ...
       X[, ncl] <- fix_freq_wrblr_int(X, j, ending.buttons = 1:4, ncl, tlim, xs, ys)
