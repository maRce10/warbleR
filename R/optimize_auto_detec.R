...
#' @param Y An object of class 'autodetec.output' (generated by \code{\link{auto_detec}}) in which to optimize detections. Must refer to the same sound files as in 'X'.
...
optimize_auto_detec <- function(X, Y, threshold = 10, power = 1, wl = 512, ssmooth = 0, hold.time = 0, mindur = NULL, maxdur = NULL, parallel = 1, pb = FALSE, by.sound.file = FALSE, bp = NULL, path = NULL, previous.output = NULL){
  ...
  if (is.null(path)) path <- getwd() else 
    if (!dir.exists(path)) stop("'path' provided does not exist") else
      ...
      if (by.sound.file)
        exp_grd <- do.call(rbind, replicate(n = length(unique(X$sound.files)), expr = exp_grd, simplify = FALSE))
