...
#' 
#' \code{sel_cut} cuts selections from a selection table into individual sound files.
#' @export sel_cut
#' @usage sel_cut(X, mar = 0.05, parallel = 1, path = NULL, dest.path = NULL, pb = TRUE,
...
#' 
#' # cut selections
#' sel_cut(lbh_selec_table, path = tempdir())
#'  
#' #check this folder!!
#' tempdir()
#' }
...
sel_cut <- function(X, mar = 0.05, parallel = 1, path = NULL, dest.path = NULL, pb = TRUE,
                     labels = c("sound.files", "selec"), overwrite = FALSE, norm = FALSE, ...){
  ...
  argms <- methods::formalArgs(sel_cut)
...
##############################################################################################################
#' alternative name for \code{\link{sel_cut}}
#'
#' @keywords internal
#' @details see \code{\link{sel_cut}} for documentation. \code{\link{cut_sels}} will be deprecated in future versions.
#' @export
