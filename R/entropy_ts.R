...
#' 
#' \code{entropy_ts} spectral entropy across signals as a time series.
#' @usage entropy_ts(X, wl = 512, length.out = 20, wn = "hanning", ovlp = 70, bp = "frange",
#'   threshold = 15, img = TRUE, parallel = 1, path = NULL, img.suffix = "entropy_ts",
#'    pb = TRUE, clip.edges = FALSE, leglab = "entropy_ts", sp.en.range = c(2, 10), ...)
...
#' The output of \code{\link{auto_detec}} can  also be used as the input data frame. 
...
#'  \code{\link{snr_specs}} for creating spectrograms to 
...
#' 
#' # without clip edges
#' entropy_ts(X = lbh_selec_table, threshold = 10, clip.edges = FALSE, length.out = 10, 
#' type = "b", sp.en.range = c(-25, 10), path = tempdir(), img = FALSE)
#' 
#' # with clip edges and length.out 10
#' entropy_ts(X = lbh_selec_table, threshold = 10, bp = c(2, 12), clip.edges = TRUE, 
...
entropy_ts <-  function(X, wl = 512, length.out = 20, wn = "hanning", ovlp = 70, 
                  bp = "frange", threshold = 15, img = TRUE, parallel = 1,
                  path = NULL, img.suffix = "entropy_ts", pb = TRUE, clip.edges = FALSE,
                  leglab = "entropy_ts", sp.en.range = c(2, 10), ...){     
  ...
  argms <- methods::formalArgs(entropy_ts)
...
##############################################################################################################
#' alternative name for \code{\link{entropy_ts}}
#'
#' @keywords internal
#' @details see \code{\link{entropy_ts}} for documentation. \code{\link{sp.en.ts}} will be deprecated in future versions.
#' @export
