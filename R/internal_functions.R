...
frd_plot_wrblr_int <- function(wave, detections, wl = 512, wn = "hanning", flim = NULL, bp = NULL, fast.spec = FALSE, ovlp = 50, pal = reverse.gray.colors.2, widths = c(2, 1), main = NULL, all.detec = F) {
  ...
  # fix flim
  if (!is.null(flim)) {
    if (flim[2] > floor(f / 2000)) flim[2] <- f / 2000
  } else {
...
frd_wrblr_int <- function(wave, wl = 512, fsmooth = 0.1, threshold = 10, wn = "hanning", bp = NULL, ovlp = 50, dB.threshold = NULL) {
  ...
  if (is.null(dB.threshold)) {
    ...
    if (!is.null(bp)) {
      ...
      } else { # select the one that contains the freq range
...
img_wrlbr_int <- function(filename, path = NULL, res = 160, units = "in", width = 8.5, height = 11, horizontal = FALSE) {
  ...
  if (grepl("jpeg$|jpg$", filename)) {
    jpeg(filename = flnm, res = res, units = units, width = width, height = height)
  } else { # or tiff
...
specprop_wrblr_int <- function(spec, f = NULL, flim = NULL, ...) {
  ...
  prec <- f / wl
  results <- list(
    mean = mean, sd = sd, median = median, sem = sem,
...
spectro_wrblr_int <- function(wave, f, wl = 512, wn = "hanning", zp = 0,
  ...
  z <- stft_wrblr_int(
    wave = wave, f = f, wl = wl, zp = zp, step = step,
  ...
  if (plot) {
    ...
    if (osc & scale) {
      ...
      par(
        las = 0, oma = oma, col = "white", col = colaxis,
        col.lab = collab, cex.lab = cexlab, cex.axis = cexaxis,
        bg = colbg
      )
      par(mar = c(0, 1, 4.5, 3))
      seewave::dBscale(
        collevels = collevels, palette = palette,
      ...
      soscillo_wrblr_int(
        wave = wave, f = f, bty = "u", from = from,
        to = to, collab = collab, colaxis = colaxis,
        colline = colaxis, ylim = c(
          -max(abs(wave)),
          max(abs(wave))
    ...
    if (osc == FALSE & scale) {
      ...
      par(
        mar = c(5, 4.1, 1, 0), las = 1, cex = 1, col = colaxis,
    ...
    if (osc & scale == FALSE) {
      ...
      par(
        mar = c(5.1, 4.1, 0, 2.1), las = 0, oma = oma,
        bg = colbg
      )
      if (!is.null(tlim) && trel) {
        wave <- wave0
      ...
      if (!fast.spec) {
        filled_contour_wrblr_int(
          ...
          }, col.lab = collab, colaxis = colaxis, bx = box, ...
        )
      } else {
    ...
    if (osc == FALSE & scale == FALSE) {
      par(
        las = 1, col = colaxis, col.axis = colaxis,
...
spectro_wrblr_int2 <- function(wave, f, wl = 512, wn = "hanning", zp = 0, ovlp = 0,
  ...
  if (!fast.spec) {
    filled_contour_wrblr_int(
      x = X, y = Y, z = Z, bg.col = colbg, levels = collevels,
      nlevels = 20, plot.title = title(
        main = main,
        xlab = tlab, ylab = flab
      ), color.palette = palette,
      axisX = FALSE, axisY = axisY, col.lab = collab,
      colaxis = colaxis, add = add
    )
  } else {
    image(x = X, y = Y, z = Z, col = palette(30), xlab = tlab, ylab = flab, axes = FALSE)
    if (!is.null(sel.tab)) {
