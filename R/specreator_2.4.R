...
specreator <- function(X, wl = 512, flim = c(0, 22), wn = "hanning", pal = rev.gray.colors.2, ovlp = 70, 
                       ...
                       gr = FALSE, sc = FALSE, line = TRUE, mar = 0.05){
  ...
  message("Creating spectrograms from selections:")
    ...
    invisible(pbapply(matrix(c(1:length(sound.files)), ncol=1), 1, function(i){
      ...
      if(propwidth){
        ...
        tiff(filename = paste(sound.files[i],"-", selec[i], "-", ".tiff", sep = ""), 
