...
#' 
#' \code{sel_tailor} produces an interactive spectrographic view in 
#' which the start/end times and frequency range of acoustic signals listed in a data frame can be adjusted.
#' @usage sel_tailor(X = NULL, wl = 512, flim = c(0,22), wn = "hanning", mar = 0.5,
...
#' 
#' sel_tailor(X =  lbh_selec_table, flim = c(1,12), wl = 300, auto.next = TRUE, path = tempdir())
#' 
...
sel_tailor <- function(X = NULL, wl = 512, flim = c(0,22), wn = "hanning", mar = 0.5,
                       ...
                       ts.df = NULL, col = "#E37222", alpha = 0.7, auto.contour = FALSE, ...)
{
  ...
  argms <- methods::formalArgs(sel_tailor)
...
##############################################################################################################
#' alternative name for \code{\link{sel_tailor}}
#'
#' @keywords internal
#' @details see \code{\link{sel_tailor}} for documentation. \code{\link{seltailor}} will be deprecated in future versions.
#' @export

seltailor <- sel_tailor
