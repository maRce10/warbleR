...
simulate_songs <- function(n = 1, durs = 0.2, harms = 3, harm.amps = c(1, 0.5, 0.2), am.amps = 1, gaps = 0.1, freqs = 5, samp.rate = 44.1, 
  ...
  if (diff.fun == "BB") df_fn <- Sim.DiffProc::BB
  ...
  if (selec.table)
  {
    if (is.null(file.name)) file.name <- gsub(" ", "_", paste0(format(Sys.time()), ".wav")) else
    ...
    options(warn = -1)
    writeWave(object = wv, filename = file.path(path, file.name), extensible = FALSE)
