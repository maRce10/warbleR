...
#' 
#' \code{sel_overlap} finds which selections overlap in time within a given sound file.
#' @usage sel_overlap(X, index = FALSE, pb = TRUE, max.ovlp = 0, relabel = FALSE, 
...
#' @return A data frame with the columns in X plus an additional column ('sel_overlap') indicating 
...
#' 
#' # drop overlapping
#'  sel_overlap(X =  Y, drop = TRUE)
#' 
#' # get index instead
#'  sel_overlap(X =  Y, index = TRUE)
#' }
...
sel_overlap <- function(X, index = FALSE, pb = TRUE, max.ovlp = 0, relabel = FALSE, drop = FALSE,                       
  ...
  argms <- methods::formalArgs(sel_overlap)
...
##############################################################################################################
#' alternative name for \code{\link{sel_overlap}}
#'
#' @keywords internal
#' @details see \code{\link{sel_overlap}} for documentation. \code{\link{ovlp_sels}} will be deprecated in future versions.
#' @export

ovlp_sels <- sel_overlap
