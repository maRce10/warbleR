...
#' 
#' \code{ff_DTW} calculates acoustic dissimilarity of fundamental frequency contours using dynamic
#' time warping. Internally it applies the \code{\link[dtw]{dtwDist}} function from the \code{dtw} package.
#' @usage ff_DTW(X, wl = 512, length.out = 20, wn = "hanning", ovlp = 70, 
...
#' @seealso \code{\link{dfts}}, \code{\link{ffts}}, \code{\link{df_DTW}}
#' @examples
#' {
#' #load data
...
#' 
#' # run function 
#' ff_DTW(lbh_selec_table[1:4,], length.out = 30, flim = c(1, 12), img = TRUE, bp = c(1, 9), wl = 300, 
#' path = tempdir())
#' }
#' 
#' @references {
...
ff_DTW <- function(X, wl = 512, length.out = 20, wn = "hanning", ovlp = 70, 
                  bp = c(0, 22), threshold = 5, img = TRUE, parallel = 1, path = NULL, 
  ...
  argms <- methods::formalArgs(ff_DTW)
  ...
  if (nrow(X) == 1) stop("you need more than one selection for ff_DTW")
...
##############################################################################################################
#' alternative name for \code{\link{ff_DTW}}
#'
#' @keywords internal
#' @details see \code{\link{ff_DTW}} for documentation. \code{\link{ffDTW}} will be deprecated in future versions.
#' @export
