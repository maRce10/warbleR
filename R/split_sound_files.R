...
split_sound_files <- function(path = NULL, sgmt.dur = 10, sgmts = NULL, files = NULL, parallel = 1, pb = TRUE, only.sels = FALSE, X = NULL) {
  ...
  if (!is.null(X)) {
    ...
    if (any(is.na(c(X$end, X$start)))) stop2("NAs found in start and/or end columns")
  ...
  if (is.null(sgmts)) {
    if (!is.numeric(sgmt.dur)) stop2("'sgmt.dur' must be numeric")
  } else if (!is.numeric(sgmts)) stop2("'sgmts' must be numeric")
