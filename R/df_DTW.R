...
#' 
#' \code{df_DTW} calculates acoustic dissimilarity of dominant frequency contours using dynamic
#' time warping. Internally it applies the \code{\link[dtw]{dtwDist}} function from the \code{dtw} package.
#' @usage df_DTW(X = NULL, wl = 512, wl.freq = 512, length.out = 20, wn = "hanning", ovlp = 70, 
...
#' 
#' # run function 
#' df_DTW(lbh_selec_table, length.out = 30, flim = c(1, 12), bp = c(2, 9), wl = 300, path = tempdir())
#' }
#' 
#' @references {
...
df_DTW <-  function(X = NULL, wl = 512, wl.freq = 512, length.out = 20, wn = "hanning", ovlp = 70, 
  ...
  argms <- methods::formalArgs(df_DTW)
  ...
  if (is.null(ts.df)) {if (nrow(X) == 1) stop("you need more than one selection for df_DTW")
...
##############################################################################################################
#' alternative name for \code{\link{df_DTW}}
#'
#' @keywords internal
#' @details see \code{\link{df_DTW}} for documentation. \code{\link{dfDTW}} will be deprecated in future versions.
#' @export
