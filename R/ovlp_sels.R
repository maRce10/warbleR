...
#' . Default is 0. 
#' @param relabel Logical. If \code{TRUE} then selections are names (selec column) are reset.  
#' Default is \code{FALSE}.
...
#' must list the levels of the column used to determine priority (argument priority.col) in the desire 
...
#' }
#' @details This function detects selections overlapping in time. Selections must be liste in a data frame similar to \code{\link{selec.table}}.
