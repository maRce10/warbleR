...
#' 
#' \code{coor_graph} creates graphs of coordinated singing and highlights the signals that overlap 
#' in time. The signals are represented by polygons of different colors.
#' @usage coor_graph(X, only.coor = FALSE, ovlp = TRUE, xl = 1, res= 80, it = "jpeg", img = TRUE, 
#' tlim = NULL, pb = TRUE)
...
#' data(sim.coor.sing)
#' 
#'#' # make coor_graph in graphic device format
#'cgs <- coor_graph(X = sim.coor.sing, ovlp = TRUE, only.coor = FALSE, img = FALSE)
#' 
#' cgs
#' }
#' 
...
coor_graph <- function(X = NULL, only.coor = FALSE, ovlp = TRUE, xl = 1,  res= 80, it = "jpeg",
                        img = TRUE, tlim = NULL, pb = TRUE) { 
  
  # error message if ggplot2 is not installed
  if (!requireNamespace("ggplot2",quietly = TRUE))
    stop("must install 'ggplot2' to use coor_graph()")
  
  #### set arguments from options
  # get function arguments
  argms <- methods::formalArgs(coor_graph)
