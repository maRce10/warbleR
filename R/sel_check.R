...
#' 
#' \code{sel_check} checks whether selections can be read by subsequent functions.
#' @usage sel_check(X, parallel = 1, path = NULL, check.header = FALSE, pb = TRUE,
...
#' 
#' sel_check(X = lbh_selec_table, path = tempdir())
#' }
...
sel_check <- function(X = NULL, parallel =  1, path = NULL, check.header = FALSE, 
  ...
  argms <- methods::formalArgs(sel_check)
  ...
  if (is_extended_selection_table(X)) stop("sel_check does not work on objects of class 'extended_selection_table'")
...
##############################################################################################################
#' alternative name for \code{\link{sel_check}}
#'
#' @keywords internal
#' @details see \code{\link{sel_check}} for documentation. \code{\link{checksels}} will be deprecated in future versions.
#' @export
...
#'
#' @keywords internal
#' @details see \code{\link{sel_check}} for documentation. \code{\link{check_sels}} will be deprecated in future versions.
#' @export

check_sels <- sel_check
