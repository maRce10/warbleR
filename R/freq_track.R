...
#' 
#' \code{freq_track} creates spectrograms to visualize dominant and fundamental frequency measurements (contours)
#' @usage freq_track(X, wl = 512, wl.freq = 512, flim = c(0, 22), wn = "hanning", pal =
...
#' @param track.harm Logical to control if \code{\link{harmonic_track}} or a modified version of \code{\link[seewave]{dfreq}} is used for dominant frequency detection. Default is \code{FALSE} (use \code{\link[seewave]{dfreq}}).
...
#'   made by \code{\link{spectro_analysis}}, \code{\link{dfts}}, \code{\link{ffts}}, \code{\link{df_DTW}} and \code{\link{ff_DTW}}. Frequency measures can be made by the function or input by the 
...
#'   Note that, unlike other warbleR functions that measure frequency contours, freq_track do not interpolate frequency values.
#' @examples
#' {
#' #load data
#' data("Cryp.soui")
...
#'
#'# now input the dfts output into freq_track         
#'freq_track(X = ad[!is.na(ad$start),], custom.contour = df ,flim = c(0, 5), ovlp = 90, it = "tiff", 
#'path = tempdir())
#' 
#'# Check this folder
#' tempdir()
...
freq_track <- function(X, wl = 512, wl.freq = 512, flim = c(0, 22), wn = "hanning", pal = reverse.gray.colors.2, ovlp = 70, 
  ...
  argms <- methods::formalArgs(freq_track)
...
{    
      dfreq1 <- harmonic_track(r, f = f, wl = wl, ovlp = 70, plot = FALSE, bandpass = b * 1000, fftw = TRUE, 
...
##############################################################################################################
#' alternative name for \code{\link{freq_track}}
#'
#' @keywords internal
#' @details see \code{\link{freq_track}} for documentation. \code{\link{trackfreqs}} will be deprecated in future versions.
#' @export

trackfreqs <- freq_track
