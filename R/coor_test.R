...
#' @usage coor_test(X, iterations = 1000, ovlp.method = "count",
...
coor_test <- function(X = NULL, iterations = 1000, ovlp.method = "count",
                      randomization = "keep.gaps", less.than.chance = TRUE, parallel = 1, pb = TRUE, 
                      rm.incomp = FALSE, cutoff = 2, rm.solo = FALSE)
{
  ...
  argms <- methods::formalArgs(coor_test)
...
##############################################################################################################
#' alternative name for \code{\link{coor_test}}
#'
#' @keywords internal
#' @details see \code{\link{coor_test}} for documentation. \code{\link{coor.test}} will be deprecated in future versions.
#' @export
