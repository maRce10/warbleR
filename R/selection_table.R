...
#' The output of \code{\link{manualoc}} or \code{\link{autodetec}} can 
...
#' This is useful for avoiding errors in downstream functions (e.g. \code{\link{specan}}, \code{\link{xcorr}}, \code{\link{catalog}}, \code{\link{dfDTW}}). Note also that corrupt files can be
...
print.extended_selection_table <- function(x, ...) {
  ...
  cat(crayon::bold(crayon::cyan(paste("Object of class 'extended_selection_table' \n"))))
  
  cat(crayon::silver(paste("contains a selection table data frame with"), nrow(x), "rows and", ncol(x), "columns: \n"))
  
    print(huxtable::set_text_color(huxtable::theme_basic(huxtable::as_hux(head(as.data.frame(x)))), "silver"))
  
  if (nrow(x) > 6) cat(crayon::silver(paste0("... and ", nrow(x) - 6, " more rows \n")))  
  
  cat(crayon::silver(paste0(length(attr(x, "wave.objects"))," wave objects (as attributes): \n")))
  ...
  cat(crayon::silver("and a data frame (check.results) generated by check_sels() (as attribute) \n"))
  
  if (attr(x, "by.song")[[1]]) cat(crayon::silver(paste0("the selection table was created", crayon::italic(crayon::bold(" by song ")), "(see 'class_extended_selection_table') \n"))) else
    cat(crayon::silver(paste0("the selection table was created", crayon::italic(crayon::bold(" by element ")), "(see 'class_extended_selection_table') \n")))

  # print number of sampling rates
  smp.rts <- unique(attr(x, "check.results")$sample.rate)
  if (length(smp.rts) == 1)
  cat(crayon::silver(paste0(length(smp.rts), " sampling rate(s): ", paste(crayon::italic(crayon::bold(smp.rts)), collapse = "/"), " \n"))) else
    cat(crayon::red(paste0(length(smp.rts), " sampling rate(s): ", paste(crayon::italic(crayon::bold(smp.rts)), collapse = "/"), " \n")))

  # print number of sampling rates
  bt.dps <- unique(attr(x, "check.results")$bits)
  if (length(bt.dps) == 1)
  cat(crayon::silver(paste0(length(bt.dps), " bit depth(s): ", paste(crayon::italic(crayon::bold(bt.dps)), collapse = "/"), " \n"))) else
    cat(crayon::red(paste0(length(bt.dps), " bit depth(s): ", paste(crayon::italic(crayon::bold(bt.dps)), collapse = "/"), " \n")))
...
print.selection_table <- function(x, ...) {
  ...
  print(huxtable::theme_basic(huxtable::as_hux(head(as.data.frame(x)))))
  
  if (nrow(x) > 6) cat(paste0("... and ", nrow(x) - 6, " more rows \n"))  
  
  cat("and a data frame (check.results) generated by check_sels() (as attribute) \n")
  
  # print warbleR version
  if (!is.null(attr(x, "warbleR.version")))
   cat(paste0("created by warbleR ", attr(x, "warbleR.version"))) else
     cat("created by warbleR < 1.1.21")
}
...
rbind.extended_selection_table <- function(..., deparse.level = 1) {
  ...
  if (any(waves.X %in% waves.Y)) cat("Some wave object names are found in both extended selection tables, they are assumed to refer to the same wave object and only one copy will be kept (use rename_waves_est() to change sound file/wave objetc names if needed)")
