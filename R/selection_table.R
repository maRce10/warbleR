...
print.extended_selection_table <- function(x, ...) {
  ...
  if (attr(x, "by.song")[[1]]) cat("the selection table was created by song (see 'class_extended_selection_table') \n") else
  ...
  cat(paste0(length(smp.rts), " sampling rate(s): ", paste(smp.rts, collapse = "/"), " \n"))

  # print number of sampling rates
  bt.dps <- unique(attr(x, "check.results")$bits)
  cat(paste0(length(bt.dps), " bit depth(s): ", paste(bt.dps, collapse = "/"), " \n"))
