...
query_xc <- function(qword, download = FALSE, X = NULL, file.name = c("Genus", "Specific_epithet"),
  ...
  if (is.null(X) & !is.null(file.name)) {
    if (any(!(file.name %in%
      ...
      stop("File name tags don't match column names in the output of this function (see documentation)")
    }
  }
  ...
  if (is.null(X)) {
    ...
    if (!is.null(file.name)) {
      if (any(!c(file.name, "recording_id") %in% tolower(colnames(X)))) {
        stop(paste(paste(c(file.name, "recording_id")[!c(file.name, "recording_id") %in% tolower(colnames(X))], collapse = ", "), "column(s) not found in data frame"))
      }
    } else if (!"Recording_ID" %in% colnames(X)) {
  ...
  if (download) {
    ...
    if (!is.null(file.name)) {
      if (length(which(tolower(names(results)) %in% file.name)) > 1) {
        fn <- apply(results[, which(tolower(names(results)) %in% file.name)], 1, paste, collapse = "-")
      } else {
