...
- min.sil.dur:
  ...
  Numeric. Controls the minimum duration of silence segments that would
  be removed.
...
- flim:
  ...
  `NULL`. Ignored if \`img = FALSE\`.

- files:

  character vector or factor indicating the subset of files that will be
  analyzed. If not provided then all wave files in the working directory
  (or path) will be processed.
...
- downsample:
  ...
  Logical indicating whether files are downsampled to a 5000 kHz
  sampling rate. This can be used to speed up the process, but will make
  the function ignore sound/noise above 2500 kHz. Default is `TRUE`.

## Value

Sound files for which silence segments have been removed are saved in
the new folder "silence-removed_files" in .wav format. If \`img = TRUE\`
then spectrogram images highlighting the silence segments that were
removed are also saved.

## Details

The function removes silence segments (i.e. segments with very low
